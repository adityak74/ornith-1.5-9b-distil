...
% Headline forcing number for the abstract; Table~\ref{tab:forcing} has the full set.
\newcommand{\Forcinghighlight}{+7.3 points of HumanEval at a 2{,}048-token budget and +3.6 at 4{,}096}

\title{Quantization Breaks Stopping Before It Breaks Reasoning:\\
Nine Attempts to Repair a 4-bit Reasoning Model, and the One That Needed No Training}

\author{%
  Aditya Karnam Gururaj Rao\\
  \texttt{adityakarnam.grao@gmail.com}\\
}
\date{}
...
recover that loss by distillation and, over twelve days on a single 64\,GB
Apple M4 Pro, tried nine things.  The first worked: two-teacher sequence-level
...
MMLU---while generating 15--33\% fewer tokens.  The next seven did not: varying
data mixture, volume, length distribution, training objective, filter, adapter
capacity, and on-policy prompt selection all failed to improve on it, and a
randomised control showed that a second training pass from the best checkpoint
costs 1--4 points regardless of what it trains on.  Diagnosing \emph{why} the
...
The ninth attempt followed from that diagnosis and needed no training at all:
budget forcing---closing the reasoning block at the decode budget and letting
the model answer---recovers \Forcinghighlight{} on the same quantized weights,
and a model forced to stop at 2{,}048 tokens outperforms the same model allowed
4{,}096.  We conclude that termination is a decoding-time property before it is
a weights property, that any benchmark of a reasoning model that does not
report its truncation rate is partly reporting its own token budget, and we
release the pipeline, every checkpoint, per-item evaluation records and a
45-section decision log.  Along the way we contribute a chunkwise-parallel
...
it.  Budget forcing \citep{muennighoff2025s1}---closing the reasoning block when
the budget is nearly spent and letting the model commit---is a decoder change,
not a training run.  It is the ninth thing we tried and, per token of budget,
the best.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item A decoding-side repair (\S\ref{sec:forcing}): budget forcing on the same
    weights, with the finding that stopping \emph{earlier} than the model would
    stop itself improves accuracy.
  \item Eight controlled negative results (\S\ref{sec:negative}), including a
    randomised control showing that continued training from the best checkpoint
    is itself harmful, and evidence that the teacher cannot supply the one
    behaviour the student lacks.
...
\section{Repairing it at decode time: budget forcing}
\label{sec:forcing}

If the failure is stopping, the cheapest test is to stop the model for it.
Following s1 \citep{muennighoff2025s1}, when the reasoning block has not closed
by the time the budget is nearly spent, we close it---appending a one-line
commit phrase and \texttt{</think>}---and allow a reserved number of tokens for
the answer.  Concretely, decoding runs in two phases: $B-N$ tokens, then $N$
...
\input{tables_forcing}

Table~\ref{tab:forcing} gives the result.  Two observations stand out.  First,
\textbf{the distilled model forced at 2{,}048 tokens outperforms the same model
allowed 4{,}096}---half the budget, more correct answers.  Forcing is not only
...
and the model does not detect it.  Wall clock is unchanged by forcing.

This was the ninth thing we tried.  It is a two-phase change to the decoder,
it generalises to any model with this failure mode including the stock build
we would have shipped without distillation, and it needed no training.
...
Table~\ref{tab:runs} lists every run after v1.  Each moved one variable, each
...
\paragraph{Scope.}
One model family, one quantization scheme, one hardware target.  The
...
this dominates the measured damage, not why.
...
A 4-bit reasoning model's dominant failure is that it does not stop.  We found
this the slow way: one distillation run that worked, seven that did not, and a
...
results into a diagnosis, and the diagnosis into the one intervention that
needed no training.  The practical recommendations are short.  Report
...
per-item evaluation records and a 45-section decision log are at \RepoURL{}.
...
\texttt{odistil eval --force-budget N} reproduces \S\ref{sec:forcing}.
