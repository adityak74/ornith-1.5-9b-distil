...
\newcommand{\Generalhighlight}{the 3-bit, 4-bit, 8-bit and full-precision builds, the 35B teacher, a dense Qwen3.8-27B and a mixture-of-experts Qwen3.6-35B all gain, by two thirds to nine tenths of whatever truncated, with one regression in 1{,}472 previously-correct items across three benchmarks; the Qwen MoE goes from 27 to 147 of 164 on the same weights and budget}
...
raises how often, so the quantized builds have the most to recover.  On
TruthfulQA, where every training run lost ground, the ramp gains 3.3 points with
no regressions.  We conclude that termination is a decoding-time property before it is a weights
...
54-section decision log.  Along the way we contribute a chunkwise-parallel
...
by 17 and the 35B teacher by 8, takes a 4-bit Qwen3.6-35B-A3B from 27 to 147 of
164 at the same budget, and across eight builds of three architectures the
gain is a fixed fraction of what truncated.  Two training-side
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item A decoding-side repair (\S\ref{sec:forcing}): \HALT{}, a harness-applied
    ...
    one regression in 1{,}472 items across eight builds and three benchmarks,
    and that the gain tracks the truncation count from 3 bits to full
    precision, from 9B to 35B, and across hybrid, dense and MoE architectures.
...
cheaper: wall clock for the full benchmark is within $\pm$10\% of plain
decoding on seven of the eight builds in Table~\ref{tab:generality} (the
eighth, bf16, compares against a plain run measured in a different session
and its token count also fell).  The processor itself is one vector add per
step.  The five items that go from wrong to right are ones that reasoned
...
Two other architectures behave the same way.  \textbf{Qwen3.8-27B}, dense,
4-bit, goes from 116 to 147 with 44 truncations, a ratio of 0.70.
\textbf{Qwen3.6-35B-A3B}, mixture-of-experts, 4-bit, solves \textbf{27 of 164}
when decoded plainly at 2{,}048 tokens because it is still reasoning at the
budget on 135 of them; with the ramp it solves \textbf{147}, the same count as
the distilled 9B and Ornith's 35B teacher, and 118 of the 135 truncations
become correct answers.  A model that benchmarks at 16\% under one decoder
benchmarks at 90\% under another, on the same weights.  Neither Qwen build has a
regression.  On TruthfulQA (Table~\ref{tab:soft}), the benchmark every training
run in \S\ref{sec:negative} lost on, the distilled 9B gains 27 of 817 with none
lost.

\input{tables_generality}
...
truncation recovered; the dashed line is 70\%.  Full precision, the dense Qwen
and the MoE Qwen sit on the same line as the quantized Ornith builds.}
...
\paragraph{Scope.}
One model family for the distillation results, one quantization scheme, one
hardware target.  The ramp was measured on two model sizes and five bit widths
of that family and on two other architectures at 4 bits; three benchmarks, one
regression in 1{,}472 items.  The
...
the two other families tested sit on the same line, but both were 4-bit
and neither was run at full precision.
...
per-item evaluation records and a 54-section decision log are at \RepoURL{}.
