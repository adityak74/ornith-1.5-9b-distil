...
\HALT{}, a Harness-Applied Limit on Thinking---close the reasoning block at the
decode budget and let the model answer---recovers \Forcinghighlight{} on the
...
it.  We call the result \HALT{}, a Harness-Applied Limit on Thinking: the
closing half of budget forcing \citep{muennighoff2025s1}, applied as a
two-phase decoder that closes the reasoning block when the budget is nearly
spent and lets the model commit.  It is a decoder change, not a training run.
It is the ninth thing we tried and, per token of budget, the best.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item A decoding-side repair (\S\ref{sec:forcing}): \HALT{}, a harness-applied
    limit on thinking, on the same weights, with the finding that stopping
    \emph{earlier} than the model would stop itself improves accuracy.
...
\caption{\HALT{} on the same weights (Table~\ref{tab:forcing}).  Grey: plain decoding under
the budget, with the number of problems truncated before answering.  Red: two-phase decoding
with a 256-token reserve; no item receives more total budget.  Dashed: the bf16 parent at
4{,}096 on the same harness.}
\label{fig:halt}
\end{figure}

Table~\ref{tab:forcing} and Figure~\ref{fig:halt} give the result.  Two observations stand out.  First,
...
\HALT{} was the ninth thing we tried.  It is a two-phase change to the decoder,
...
\texttt{odistil eval --force-budget N} reproduces \HALT{} (\S\ref{sec:forcing}).
