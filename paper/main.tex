...
full-precision parent at 4{,}096.  We conclude that termination is a decoding-time property before it is
...
52-section decision log.  Along the way we contribute a chunkwise-parallel
...
HumanEval problems on the stock 4-bit build with no regressions, and two
training-side attempts to move the same behaviour into the weights both
failed---which is itself evidence about where the behaviour lives.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item A decoding-side repair (\S\ref{sec:forcing}): \HALT{}, a harness-applied
    ...
    model would stop itself improves accuracy and that the soft form does so
    with zero regressions across three benchmark--build pairs.
...
Our reading is that quantization did not remove the model's ability to stop.
It lowered the \texttt{</think>} logit relative to continuing at the positions
where the decision is due, by an amount that grows with how long the model has
been reasoning.  A bias that grows with position restores the decision without
making it for the model; the hard form was the coarse version of the same
repair.  Two attempts to move this repair into the weights are reported in
\S\ref{sec:v9v10}; both failed, which is consistent with the behaviour being a
property of the quantized forward pass rather than of anything the training
data can teach.
...
the model choose---is the better and the simpler.  The practical recommendations are short.  Report
...
per-item evaluation records and a 52-section decision log are at \RepoURL{}.
